...
const Sidebar = () => {
  ...
  const fetchCanvases = async () => {
    try {
      const response = await axios.get('http://localhost:5001/api/canvas/list', {
  ...
  const handleCreateCanvas = async () => {
    try {
      const response = await axios.post('http://localhost:5001/api/canvas/create', {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log(response.data)  
      fetchCanvases();
  ...
  const handleDeleteCanvas = async (id) => {
    try {
      await axios.delete(`http://localhost:5001/api/canvas/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCanvases();
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5001/api/canvas/share/${canvasId}`,
        { email },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
